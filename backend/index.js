...
app.get('/api/dashboard/summary', async (req, res) => {
    try {
        ...
        // Pipeline agregasi ini lebih tangguh dalam menangani format tanggal yang berbeda
        const pipeline = [
            {
                                        ...
                                        $cond: {
                                            ...
                                            then: { $year: "$$dateField" }, // Jika formatnya sudah Date
                                                ...
                                                $cond: {
                                                    ...
                                                    then: { $toInt: { $substrCP: ["$$dateField", 0, 4] } },
                                                    // Jika formatnya 'D/M/YYYY' atau lainnya
                                                    else: { $toInt: { $arrayElemAt: [{ $split: ["$$dateField", "/"] }, -1] } }
                                                }
                                            }
                                        }
                                    },
