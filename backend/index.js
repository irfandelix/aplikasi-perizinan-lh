...
app.post('/api/submit/:tahap', async (req, res) => {
    ...
    try {
        ...
        // --- LOGIKA TAHAP E YANG DIPERBAIKI ---
        else if (tahap === 'e') {
            const { tanggalRevisi, nomorRevisi } = req.body;

            // --- PERBAIKAN: Gunakan 'existingData' ---
            ...
            if (!existingData.tanggalPemeriksaan) return res.status(400).json({ success: false, message: 'Gagal: Tanggal Pemeriksaan (Tahap D) tidak ditemukan.' });
            ...
            // --- PERBAIKAN: Gunakan 'existingData' ---
            const tglPartsLama = getDateParts(existingData.tanggalPemeriksaan); 
            const tglPartsBaru = getDateParts(tanggalRevisi); 
            
            const nomorSetelahUpdateTanggal = baseNomor
                .replace(`.${tglPartsLama.month}/`, `.${tglPartsBaru.month}/`)
                .replace(`/${tglPartsLama.year}`, `/${tglPartsBaru.year}`);
